...
$parentModule = 'FileType'
if (-not $ModuleName) { $ModuleName = 'FileType.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\FileType'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\FileType.psd1"
$prereqHash = @{
